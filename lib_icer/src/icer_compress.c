...
static inline int comp_packet(const void *a, const void *b) {
    if (((icer_packet_context_typedef *)a)->priority == ((icer_packet_context_typedef *)b)->priority) {
        ...
        return (((icer_packet_context_typedef *)a)->subband_type > ((icer_packet_context_typedef *)b)->subband_type) ? 1 :
               (((icer_packet_context_typedef *)a)->subband_type < ((icer_packet_context_typedef *)b)->subband_type) ? -1 : 0;
    }
    return (((icer_packet_context_typedef *)a)->priority > ((icer_packet_context_typedef *)b)->priority) ? -1 : 1;
}
...
int icer_compress_image_uint8(uint8_t * const image, size_t image_w, size_t image_h, uint8_t stages, enum icer_filter_types filt,
    ...
    for (uint8_t curr_stage = 1;curr_stage <= stages;curr_stage++) {
        ...
        for (uint8_t lsb = 0;lsb < ICER_BITPLANES_TO_COMPRESS_8;lsb++) {
            ...
            SET_PKT_CONTEXT_MACRO(icer_packets[ind], ll_mean, lsb, chan, lsb == (ICER_BITPLANES_TO_COMPRESS_8 - 1))
...
int icer_decompress_image_uint8(uint8_t * const image, size_t * const image_w, size_t * const image_h, const size_t image_bufsize, const uint8_t *datastream,
    ...
    while ((data_length - offset) > 0) {
        ...
        if (res == ICER_RESULT_OK) {
            icer_reconstruct_data_8[chan][seg->decomp_level][ICER_GET_SUBBAND_TYPE_MACRO(seg->metadata_subband_type)][seg->segment_number][ICER_GET_LSB_MACRO(seg->lsb_chan)] = seg;
...
int icer_compress_image_uint16(uint16_t * const image, size_t image_w, size_t image_h, uint8_t stages, enum icer_filter_types filt,
    ...
    for (uint8_t curr_stage = 1;curr_stage <= stages;curr_stage++) {
        ...
        for (uint8_t lsb = 0;lsb < ICER_BITPLANES_TO_COMPRESS_16;lsb++) {
            ...
            SET_PKT_CONTEXT_MACRO(icer_packets_16[ind], ll_mean, lsb, chan, lsb == (ICER_BITPLANES_TO_COMPRESS_16 - 1))
    ...
    for (size_t it = 0;it < ind;it++) {
        ...
        icer_generate_partition_parameters(&partition_params, ll_w, ll_h, segments);
        res = icer_compress_partition_uint16(data_start, &partition_params, image_w, &(icer_packets_16[it]), &metadata,
...
int icer_decompress_image_uint16(uint16_t * const image, size_t * const image_w, size_t * const image_h, size_t image_bufsize, const uint8_t *datastream,
    ...
    while ((data_length - offset) > 0) {
        ...
        res = icer_find_packet_in_bytestream(&seg, seg_start, data_length - offset, &pkt_offset);
        if (res == ICER_RESULT_OK) {
            icer_reconstruct_data_16[chan][seg->decomp_level][ICER_GET_SUBBAND_TYPE_MACRO(seg->metadata_subband_type)][seg->segment_number][ICER_GET_LSB_MACRO(seg->lsb_chan)] = seg;
            ll_mean = seg->ll_mean_val;
            if (ICER_GET_METADATA_FLAG_MACRO(seg->metadata_subband_type)) {
                metadata = (icer_image_segment_metadata_typedef *) (seg + 1);
                *image_w = metadata->image_w;
...
int icer_find_packet_in_bytestream(const icer_image_segment_typedef **seg, const uint8_t *datastream, size_t data_length, size_t * const offset) {
    (*offset) = 0;
    (*seg) = NULL;
