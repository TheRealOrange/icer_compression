//
// Created by Lin Yicheng on 4/8/25.
//
...
void print_usage(const char *program_name) {
  ...
  printf("  -c, --color           Use color mode (YUV)\n");
  printf("  -G, --grayscale       Use grayscale mode\n");
  printf("\n");
  printf("compression-only options:\n");
  printf("  -t, --size <size>     Target compressed size in bytes (default: lossless)\n");
  printf("\n");
  printf("others:\n");
  printf("  --help                Show this help message\n\n");
  printf("You must specify either --color or --grayscale for decompression operations\n");
  printf("  If not using default compression parameters, parameters (stages, filter, segments) must be specified for decompression\n");
}
...
int decompress_image(icer_config_t *config) {
  ...
  if (dim_result != ICER_RESULT_OK) {
    fprintf(stderr, "Error: Could not determine image dimensions from compressed data\n");
    result = 1;
    goto cleanup;
  }
  ...
  printf("Using parameters: stages=%d, filter=%d, segments=%d\n",
         config->stages, config->filter, config->segments);
  ...
  if (decompress_result != ICER_RESULT_OK) {
    ...
    fprintf(stderr, "Make sure the compression parameters match those used for compression\n");
    result = 1;
    goto cleanup;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (!config.force_color && !config.force_grayscale && strcmp(config.operation, "decompress") == 0) {
    fprintf(stderr, "Error: You must specify either --color or --grayscale for decompression\n");
    return 1;
  }
